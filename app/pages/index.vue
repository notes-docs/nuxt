...
const [{ data: page }, { data: officialModules }, { data: sponsorGroups }] = await Promise.all([
  ...
  useAsyncData('index', () => queryCollection('index').first()), // 首页内容数据
  useFetch('https://api.nuxt.com/modules', {
    key: 'official-modules',
    transform: (res: { modules: Module[], stats: Stats }) => res.modules
      .filter(module => module.type === 'official') // 只保留官方模块
      .sort((a, b) => b.stats.stars - a.stats.stars) // 按星数降序
  }),
  useFetch('https://api.nuxt.com/sponsors', {
    key: 'top-sponsors',
    transform: (sponsors: Record<string, Sponsor[]>) => Object.entries(sponsors)
      .filter(([tier]) => ['diamond', 'platinum', 'gold'].includes(tier)) // 过滤高级赞助商
      .map(([tier, sponsors]) => ({ // 将原始对象转换为 { tier, sponsors[] } 数组，便于渲染
        tier,
...
// 当前选中的 bundler 索引（用于切换打包工具介绍）
const activeBundlerIndex = ref(0)

// 处理 foundation 区块的条目布局，对 bundler 类型做特殊处理
...
<template>
  <div v-if="page">
    ...
    >
      ...
      <template #title>
        渐进式<br><span class="text-primary">Web Framework</span>
      </template>
